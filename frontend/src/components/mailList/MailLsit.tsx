...
export default function MailList({ page }: { page: number }) {
  ...
  const emailCount = selectedFolder.email_count || 0;
    ...
    <div style={{ width: "100%" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
        <tbody>
          {showSkeleton &&
            [...Array(20)].map((_, i) => (
              <tr key={`skeleton-${i}`} style={{ backgroundColor: "#f6f6f6", animation: "pulse 1.5s infinite" }}>
          ...
          {!loading &&
            emails.map((email) => (
              <tr
                key={email.uid}
                onClick={() => navigate(`/emails?account_id=${account.id}&uid=${email.uid}`)}
              ...
              >
                ...
                <td style={{ padding: 8, width: "5%" }} onClick={(e) => {
                  e.stopPropagation(); // чтобы не переходить по письму
                  toggleStar(email.uid);
                }}>
                  {email.is_starred ? (
                    <FaStar size={16} color="#fdd835" />
                  ) : (
                    <FaRegStar size={16} color="#999" />
                  )}
                </td>
                <td style={{ padding: 8, width: "5%" }}>
                  <FaChevronRight size={14} color="#999" />
                </td>
                <td style={{
                  padding: 8, width: "25%",
                ...
                <td style={{
                  ...
                  <span style={{ fontWeight: 500, color: "#000" }}>{email.subject || "(без темы)"}</span>
                  <span style={{ color: "#666" }}> – {email.snippet}</span>
                </td>
                <td style={{ padding: 8, width: "10%", color: "#666", fontSize: 13 }}>
